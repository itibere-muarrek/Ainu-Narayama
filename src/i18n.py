...
T: dict[str, dict[str, str]] = {
    ...
    "narayama_intro": {
        "pt": "Acompanhe, de forma simples, a capacidade de renovação geracional de 7 países ao final de um ciclo de 25 anos — com base na tese de doutorado \"Do Dilema de Narayama ao Oicoceno Civilizacional\" (v8.0). Para análises detalhadas, simulações e calibração, veja [ainu.systems](https://ainu.systems).",
        "en": "Track, in simple terms, the generational renewal capacity of 7 countries at the end of a 25-year cycle — based on the doctoral thesis \"From Narayama's Dilemma to the Civilizational Oikocene\" (v8.0). For detailed analysis, simulations and calibration, see [ainu.systems](https://ainu.systems).",
        "es": "Siga, de forma simple, la capacidad de renovación generacional de 7 países al final de un ciclo de 25 años — según la tesis doctoral \"Del Dilema de Narayama al Oicoceno Civilizacional\" (v8.0). Para análisis detallados, simulaciones y calibración, vea [ainu.systems](https://ainu.systems).",
        "ja": "博士論文『ナラヤマのジレンマから文明的オイコセーンへ』（v8.0）に基づき、25年周期の終わりにおける7か国の世代更新能力をシンプルに追跡します。詳細な分析やシミュレーション、キャリブレーションについては[ainu.systems](https://ainu.systems)をご覧ください。",
        "ko": "박사 학위논문 『나라야마의 딜레마에서 문명적 오이코세까지』(v8.0)를 바탕으로, 25년 주기가 끝나는 시점의 7개국 세대 재생산 능력을 간단히 살펴봅니다. 자세한 분석, 시뮬레이션, 보정은 [ainu.systems](https://ainu.systems)에서 확인하세요.",
        "it": "Segui, in modo semplice, la capacità di rinnovamento generazionale di 7 paesi al termine di un ciclo di 25 anni — sulla base della tesi di dottorato \"Dal Dilemma di Narayama all'Oicocene Civilizzazionale\" (v8.0). Per analisi dettagliate, simulazioni e calibrazione, vedi [ainu.systems](https://ainu.systems).",
        "fi": "Seuraa yksinkertaisesti 7 maan sukupolvien uusiutumiskykyä 25 vuoden syklin lopussa — perustuen väitöskirjaan \"Narayaman dilemmasta sivilisaation Oikoseeniin\" (v8.0). Yksityiskohtaista analyysia, simulaatioita ja kalibrointia varten katso [ainu.systems](https://ainu.systems).",
        "fr": "Suivez, de façon simple, la capacité de renouvellement générationnel de 7 pays à la fin d'un cycle de 25 ans — d'après la thèse de doctorat \"Du Dilemme de Narayama à l'Oïcocène Civilisationnel\" (v8.0). Pour des analyses détaillées, des simulations et un calibrage, voir [ainu.systems](https://ainu.systems).",
        "zh": "基于博士论文《从楢山难题到文明世的宜居纪》(v8.0)，简明呈现7个国家在25年周期结束时的世代更新能力。详细分析、模拟与校准请见[ainu.systems](https://ainu.systems)。",
    },
    "narayama_info": {
        "pt": "**Como ler o N\\*:** não é uma nota de \"quanto maior, melhor\" — é um equilíbrio. N\\* muito baixo indica **Colapso de Narayama (PEC)**: poucos nascimentos não repõem a geração legatária. N\\* muito alto indica **Saturação por Overbirths (PEEC)**: crescimento populacional acelerado além da capacidade de absorção institucional. O ideal fica no meio da escala, faixa chamada de **Equilíbrio Sustentável (PEA)**. O farol institucional (+/n/-) ainda não aparece aqui: depende de dados de Contas de Transferências Nacionais (NTA) que este projeto ainda não tem — ver [ainu.systems](https://ainu.systems) para o detalhamento completo da metodologia.",
        "en": "**How to read N\\*:** it's not a \"higher is better\" score — it's a balance. A very low N\\* indicates **Narayama Collapse (PEC)**: too few births to replace the legacy generation. A very high N\\* indicates **Overbirth Saturation (PEEC)**: population growth accelerating beyond institutional absorption capacity. The ideal sits in the middle of the scale, a range called **Sustainable Equilibrium (PEA)**. The institutional beacon (+/n/-) doesn't appear here yet: it depends on National Transfer Accounts (NTA) data this project doesn't have yet — see [ainu.systems](https://ainu.systems) for the full methodology.",
        "es": "**Cómo leer el N\\*:** no es una nota de \"cuanto más alto, mejor\" — es un equilibrio. Un N\\* muy bajo indica **Colapso de Narayama (PEC)**: pocos nacimientos no reponen a la generación legataria. Un N\\* muy alto indica **Saturación por Overbirths (PEEC)**: crecimiento poblacional acelerado más allá de la capacidad de absorción institucional. Lo ideal está en el medio de la escala, franja llamada **Equilibrio Sostenible (PEA)**. El farol institucional (+/n/-) todavía no aparece aquí: depende de datos de Cuentas de Transferencias Nacionales (NTA) que este proyecto aún no tiene — ver [ainu.systems](https://ainu.systems) para el detalle completo de la metodología.",
        "ja": "**N\\*の読み方：** 「高いほど良い」という指標ではなく、バランスを示します。N\\*が非常に低い場合は **ナラヤマ崩壊(PEC)** を示します：出生数が少なすぎて先代世代を置き換えられません。N\\*が非常に高い場合は **オーバーバース飽和(PEEC)** を示します：人口増加が制度的な受容能力を超えて加速しています。理想は尺度の中央、**持続可能な均衡(PEA)** と呼ばれる範囲にあります。制度的ビーコン（+/n/-）はまだここには表示されません：本プロジェクトがまだ保有していないNational Transfer Accounts（NTA）のデータに依存するためです — 詳しい方法論は[ainu.systems](https://ainu.systems)をご覧ください。",
        "ko": "**N\\* 읽는 법:** \"높을수록 좋다\"는 점수가 아니라 균형을 나타냅니다. N\\*가 매우 낮으면 **나라야마 붕괴(PEC)**를 의미합니다: 출생아 수가 너무 적어 이전 세대를 대체하지 못합니다. N\\*가 매우 높으면 **과잉출생 포화(PEEC)**를 의미합니다: 인구 증가가 제도적 수용 능력을 넘어 가속화됩니다. 이상적인 값은 척도의 중간, **지속가능한 균형(PEA)**이라 불리는 구간에 있습니다. 제도적 신호등(+/n/-)은 아직 여기에 표시되지 않습니다: 이 프로젝트가 아직 보유하지 않은 국민이전계정(NTA) 데이터에 의존하기 때문입니다 — 전체 방법론은 [ainu.systems](https://ainu.systems)를 참고하세요.",
        "it": "**Come leggere l'N\\*:** non è un punteggio \"più alto è meglio\" — è un equilibrio. Un N\\* molto basso indica **Collasso di Narayama (PEC)**: poche nascite non sostituiscono la generazione legataria. Un N\\* molto alto indica **Saturazione da Overbirths (PEEC)**: crescita demografica accelerata oltre la capacità di assorbimento istituzionale. L'ideale sta nel mezzo della scala, fascia chiamata **Equilibrio Sostenibile (PEA)**. Il faro istituzionale (+/n/-) non appare ancora qui: dipende dai dati dei National Transfer Accounts (NTA) che questo progetto non ha ancora — vedi [ainu.systems](https://ainu.systems) per il dettaglio completo della metodologia.",
        "fi": "**Miten N\\*:ää luetaan:** kyse ei ole \"mitä korkeampi, sitä parempi\" -pisteestä — vaan tasapainosta. Hyvin matala N\\* tarkoittaa **Narayama-romahdusta (PEC)**: liian vähän syntyvyyttä korvaamaan edellinen sukupolvi. Hyvin korkea N\\* tarkoittaa **ylisyntyvyyden kyllästymää (PEEC)**: väestönkasvu kiihtyy yli institutionaalisen vastaanottokyvyn. Ihanne on asteikon keskellä, alueella nimeltä **kestävä tasapaino (PEA)**. Institutionaalinen valo (+/n/-) ei näy vielä tässä: se riippuu National Transfer Accounts (NTA) -tiedoista, joita tällä projektilla ei vielä ole — katso [ainu.systems](https://ainu.systems) koko menetelmä.",
        "fr": "**Comment lire le N\\* :** ce n'est pas une note \"plus c'est haut, mieux c'est\" — c'est un équilibre. Un N\\* très bas indique un **Effondrement de Narayama (PEC)** : trop peu de naissances pour remplacer la génération léguée. Un N\\* très élevé indique une **Saturation par surnatalité (PEEC)** : croissance démographique accélérée au-delà de la capacité d'absorption institutionnelle. L'idéal se situe au milieu de l'échelle, une plage appelée **Équilibre durable (PEA)**. Le repère institutionnel (+/n/-) n'apparaît pas encore ici : il dépend des données des National Transfer Accounts (NTA), que ce projet n'a pas encore — voir [ainu.systems](https://ainu.systems) pour le détail complet de la méthodologie.",
        "zh": "**如何解读N\\*：** 这不是“越高越好”的分数，而是一种平衡。N\\*过低表示 **楢山崩溃(PEC)** ：出生人数过少，无法替代上一代人。N\\*过高表示 **过度生育饱和(PEEC)** ：人口增长速度超过了制度吸纳能力。理想值位于量表中段，称为 **可持续平衡(PEA)** 区间。制度信号灯（+/n/-）此处尚未显示：它依赖于本项目尚未获取的国民转移账户（NTA）数据 — 完整方法论请见[ainu.systems](https://ainu.systems)。",
    },
    ...
    "footer_narayama": {
        "pt": "**Dados**: UN World Population Prospects + OECD Social Expenditure Database  \n**Metodologia**: Tese \"Do Dilema de Narayama ao Oicoceno Civilizacional\" v8.0  \n**Análise detalhada**: [ainu.systems](https://ainu.systems)",
        "en": "**Data**: UN World Population Prospects + OECD Social Expenditure Database  \n**Methodology**: Thesis \"From Narayama's Dilemma to the Civilizational Oikocene\" v8.0  \n**Detailed analysis**: [ainu.systems](https://ainu.systems)",
        "es": "**Datos**: UN World Population Prospects + OECD Social Expenditure Database  \n**Metodología**: Tesis \"Del Dilema de Narayama al Oicoceno Civilizacional\" v8.0  \n**Análisis detallado**: [ainu.systems](https://ainu.systems)",
        "ja": "**データ**: UN World Population Prospects + OECD Social Expenditure Database  \n**方法論**: 博士論文『ナラヤマのジレンマから文明的オイコセーンへ』v8.0  \n**詳細分析**: [ainu.systems](https://ainu.systems)",
        "ko": "**데이터**: UN World Population Prospects + OECD Social Expenditure Database  \n**방법론**: 논문 『나라야마의 딜레마에서 문명적 오이코세까지』 v8.0  \n**상세 분석**: [ainu.systems](https://ainu.systems)",
        "it": "**Dati**: UN World Population Prospects + OECD Social Expenditure Database  \n**Metodologia**: Tesi \"Dal Dilemma di Narayama all'Oicocene Civilizzazionale\" v8.0  \n**Analisi dettagliata**: [ainu.systems](https://ainu.systems)",
        "fi": "**Data**: UN World Population Prospects + OECD Social Expenditure Database  \n**Menetelmä**: Väitöskirja \"Narayaman dilemmasta sivilisaation Oikoseeniin\" v8.0  \n**Yksityiskohtainen analyysi**: [ainu.systems](https://ainu.systems)",
        "fr": "**Données** : UN World Population Prospects + OECD Social Expenditure Database  \n**Méthodologie** : Thèse \"Du Dilemme de Narayama à l'Oïcocène Civilisationnel\" v8.0  \n**Analyse détaillée** : [ainu.systems](https://ainu.systems)",
        "zh": "**数据**：UN World Population Prospects + OECD Social Expenditure Database  \n**方法论**：论文《从楢山难题到文明世的宜居纪》v8.0  \n**详细分析**：[ainu.systems](https://ainu.systems)",
    },
    ...
    "ainu_intro": {
        "pt": "Cálculos detalhados do Índice de Narayama Sistêmico (N*) para os 28 países da amostra, com base na tese de doutorado \"Do Dilema de Narayama ao Oicoceno Civilizacional\" (v8.0). Para a visão pública e simplificada, veja [narayama.live](https://narayama.live).",
        "en": "Detailed calculations of the Systemic Narayama Index (N*) for the 28 countries in the sample, based on the doctoral thesis \"From Narayama's Dilemma to the Civilizational Oikocene\" (v8.0). For the public, simplified view, see [narayama.live](https://narayama.live).",
        "es": "Cálculos detallados del Índice de Narayama Sistémico (N*) para los 28 países de la muestra, según la tesis doctoral \"Del Dilema de Narayama al Oicoceno Civilizacional\" (v8.0). Para la vista pública y simplificada, vea [narayama.live](https://narayama.live).",
        "ja": "博士論文『ナラヤマのジレンマから文明的オイコセーンへ』（v8.0）に基づく、サンプル28か国のシステミック・ナラヤマ指数（N*）の詳細な計算です。公開・簡易版は[narayama.live](https://narayama.live)をご覧ください。",
        "ko": "박사 학위논문 『나라야마의 딜레마에서 문명적 오이코세까지』(v8.0)를 바탕으로 한 표본 28개국의 체계적 나라야마 지수(N*) 상세 계산입니다. 공개용 간략 버전은 [narayama.live](https://narayama.live)를 참고하세요.",
        "it": "Calcoli dettagliati dell'Indice di Narayama Sistemico (N*) per i 28 paesi del campione, sulla base della tesi di dottorato \"Dal Dilemma di Narayama all'Oicocene Civilizzazionale\" (v8.0). Per la vista pubblica e semplificata, vedi [narayama.live](https://narayama.live).",
        "fi": "Yksityiskohtaiset laskelmat systeemisestä Narayama-indeksistä (N*) otoksen 28 maalle, perustuen väitöskirjaan \"Narayaman dilemmasta sivilisaation Oikoseeniin\" (v8.0). Julkista, yksinkertaistettua näkymää varten katso [narayama.live](https://narayama.live).",
        "fr": "Calculs détaillés de l'indice de Narayama systémique (N*) pour les 28 pays de l'échantillon, d'après la thèse de doctorat \"Du Dilemme de Narayama à l'Oïcocène Civilisationnel\" (v8.0). Pour la vue publique simplifiée, voir [narayama.live](https://narayama.live).",
        "zh": "基于博士论文《从楢山难题到文明世的宜居纪》(v8.0)，对样本中28个国家的系统性楢山指数（N*）进行详细计算。公开简化版请见[narayama.live](https://narayama.live)。",
    },
