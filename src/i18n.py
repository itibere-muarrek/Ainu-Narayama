...
T: dict[str, dict[str, str]] = {
    ...
    "farol_provisorio_caption": {
        "pt": "Nos 8 países também destacados no narayama.live, existe um valor de farol provisório herdado do modelo demográfico-TFR anterior à atualização do pipeline (Tabela 3.1.1 da tese) — mas a decisão foi não publicá-lo: não vem de fonte real de National Transfer Accounts (NTA). Por isso o farol aparece sempre como `s/d` nos cards, mesmo para esses 8 países.",
        "en": "For the 8 countries also featured on narayama.live, a provisional beacon value exists, inherited from the demographic-TFR model that predates the pipeline update (thesis Table 3.1.1) — but the decision was not to publish it: it doesn't come from a real National Transfer Accounts (NTA) source. That's why the beacon always shows as `n/a` on the cards, even for these 8 countries.",
        "es": "Para los 8 países también destacados en narayama.live, existe un valor de farol provisional heredado del modelo demográfico-TFR anterior a la actualización del pipeline (Tabla 3.1.1 de la tesis) — pero la decisión fue no publicarlo: no proviene de una fuente real de Cuentas de Transferencias Nacionales (NTA). Por eso el farol aparece siempre como `s/d` en las tarjetas, incluso para esos 8 países.",
        "ja": "narayama.liveでも注目されている8か国については、パイプライン更新前の人口統計-TFRモデルから引き継いだ暫定的なビーコン値が存在します（論文の表3.1.1）— しかし、それを公開しないという決定がなされました：実際のNational Transfer Accounts（NTA）のデータ源に基づくものではないためです。そのため、この8か国についても、カードのビーコンは常にN/Aと表示されます。",
        "ko": "narayama.live에서도 강조되는 8개국에 대해서는, 파이프라인 업데이트 이전의 인구통계-TFR 모델에서 물려받은 잠정적인 신호등 값이 존재합니다(논문 표 3.1.1) — 하지만 이를 공개하지 않기로 결정했습니다: 실제 국민이전계정(NTA) 출처에서 나온 것이 아니기 때문입니다. 그래서 이 8개국에 대해서도 카드의 신호등은 항상 N/A로 표시됩니다.",
        ...
        "zh": "在narayama.live同样重点展示的8个国家中，存在一个继承自流水线更新之前人口统计-TFR模型的临时信号灯数值（论文表3.1.1）——但决定是不公开它：因为它并非来自真实的国民转移账户（NTA）来源。因此，即使是这8个国家，卡片上的信号灯也始终显示为N/A。",
    },
    ...
    "tabela_geracional_caption_ainu": {
        ...
        "en": "Comparative grid of the 28 countries by N\\* (thesis Section 9-A.7, 2026-07-12 revision). Horizontal axis = Narayama Index zone (Population Excess/PEEC → Collapse/PEC); vertical axis, within each zone = current population, largest at the top. Always shows all 28 countries, regardless of the Profile/Region filters above.",
        ...
        "ja": "N\\*による28か国の比較グリッド（論文セクション9-A.7、2026年7月12日改訂）。横軸＝ナラヤマ指数のゾーン（人口過剰/PEEC → 崩壊/PEC）、各ゾーン内の縦軸＝現在の人口（上ほど多い）。上部のプロファイル/地域フィルターにかかわらず、常に28か国すべてを表示します。",
        "ko": "N\\*에 따른 28개국 비교 그리드(논문 9-A.7절, 2026-07-12 개정). 가로축 = 나라야마 지수 구역(과잉 인구/PEEC → 붕괴/PEC); 각 구역 내 세로축 = 현재 인구(위로 갈수록 많음). 위의 프로필/지역 필터와 무관하게 항상 28개국 전체를 표시합니다.",
        ...
        "fr": "Grille comparative des 28 pays selon le N\\* (Section 9-A.7 de la thèse, révision du 12/07/2026). Axe horizontal = zone de l'indice de Narayama (Excès démographique/PEEC → Effondrement/PEC) ; axe vertical, au sein de chaque zone = population actuelle, la plus grande en haut. Affiche toujours les 28 pays, indépendamment des filtres Profil/Région ci-dessus.",
        "zh": "按N\\*划分的28个国家比较网格（论文第9-A.7节，2026年7月12日修订）。横轴=楢山指数区域（人口过剩/PEEC → 崩溃/PEC）；纵轴（各区域内）=当前人口，越靠上人口越多。无论上方的类型/地区筛选如何，始终显示全部28个国家。",
    },
    "farol_gap_caption": {
    ...
    "glossario_intro": {
        "pt": "Todos os termos e siglas da tese, em ordem de aparição — inclui conceitos de fases futuras ainda não implementadas neste projeto (Anexo 14 da tese V12, com a correção dos limiares de PEC/PEA/PEEC pra convenção adotada de 5 zonas, ver docs/definitions.md).",
        "en": "All terms and acronyms from the thesis, in order of appearance — includes concepts from future phases not yet implemented in this project (thesis V12, Annex 14, with PEC/PEA/PEEC thresholds corrected to the adopted 5-zone convention — see docs/definitions.md).",
        "es": "Todos los términos y siglas de la tesis, en orden de aparición — incluye conceptos de fases futuras aún no implementadas en este proyecto (Anexo 14 de la tesis V12, con la corrección de los umbrales de PEC/PEA/PEEC a la convención adoptada de 5 zonas, ver docs/definitions.md).",
        "ja": "論文のすべての用語と略語を、登場順に記載 — 本プロジェクトではまだ実装されていない将来フェーズの概念も含む（論文V12、附属書14、PEC/PEA/PEECの閾値は採用された5ゾーン規約に修正済み — docs/definitions.md参照）。",
        "ko": "논문의 모든 용어와 약어를 등장 순서대로 수록 — 이 프로젝트에서 아직 구현되지 않은 향후 단계의 개념도 포함(논문 V12, 부록 14, PEC/PEA/PEEC 임계값은 채택된 5구역 규약으로 수정됨 — docs/definitions.md 참고).",
        "it": "Tutti i termini e le sigle della tesi, in ordine di apparizione — include concetti di fasi future non ancora implementate in questo progetto (Allegato 14 della tesi V12, con la correzione delle soglie PEC/PEA/PEEC alla convenzione adottata a 5 zone, vedi docs/definitions.md).",
        ...
        "fr": "Tous les termes et acronymes de la thèse, dans l'ordre d'apparition — inclut des concepts de phases futures pas encore implémentées dans ce projet (Annexe 14 de la thèse V12, avec les seuils PEC/PEA/PEEC corrigés selon la convention adoptée à 5 zones, voir docs/definitions.md).",
        "zh": "论文中所有术语和缩写，按出现顺序排列——包括本项目尚未实现的未来阶段概念（论文V12附录14，其中PEC/PEA/PEEC阈值已按采用的5区域惯例更正——见docs/definitions.md）。",
    },
    "footer_ainu": {
